...
const ORDER_REQUIREMENTS = {
  ...
  // 幸运药水订单需求: 药水(24)
  lucky_potion(ctx) {
    return equalFrac(ctx.cards[0].value, intToFrac(24));
  },
  //
  potion_35(ctx) {
    return equalFrac(ctx.cards[0].value, intToFrac(35));
  },
  //
  the_17th_element(ctx) {
    return equalFrac(ctx.cards[0].value, intToFrac(79));
  },
  // 方体吸附剂订单需求: 药水(2)(4)(8).
  cube_absorber(ctx) {
    return (
...
const ORDER_REWARDS = {
  ...
  // 蒙面人的请求奖励
  masked_request(ctx) {
    if (ctx.state.hand.length===0) return;
    const c=ctx.state.hand[0];
    gainCard({
      type:c.type,
      suit:c.suit,
      value:{...c.value}
...
// 根据订单序号 orderIndex 和 uiState 交付订单, 判定交付成功与否: 成功则订单与所交付牌消失并获得奖励; 失败则不发生任何事
function submitOrder(orderIndex, uiState) {

  if (!canDoAction("castSpell")) return;

  // 根据订单序号 orderIndex 在订单列 orders 找到正在被提交的订单, 记作 order
  ...
  // 触发事件***
  emitEvent("orderCompleted", { order });
  onGameEvent("orderCompleted", { order });
  // 检查是否胜利
  checkVictory();
  // 刷新反应台状态
  state.reactor = {
...
function submitAssociationOrder(cardIndex) {
  ...
  if (value.den === 1 && value.num === state.associationOrder.requirement) {
    // 触发协会订单的奖励效果
    state.associationOrder.rewardEffect();
